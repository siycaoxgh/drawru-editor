...
### 2.5 推荐卡片禁止使用 padding-top 占位

**规则：** 推荐阅读卡片可以使用 16:6 比例和 `object-fit: cover`，但不要使用 `padding-top: 37.5%` 制造比例空间。

**原因：** 微信编辑器保存 HTML 时可能移除图片的 `position: absolute`，但保留外层的 `padding-top`。此时 padding 会变成图片前方的真实空白区域。

**推荐结构：**
```html
<section style="width:100%; aspect-ratio:16/6; overflow:hidden; line-height:0;">
    <img style="width:100%; height:100%; object-fit:cover; display:block;">
</section>
```

该规则只适用于 RecommendCard。正文图片必须使用自然比例：`width:100%; height:auto;`。

## 3. 图片资源规则

### 3.1 分割线使用零尺寸 SVG 占位符

**规则：** Divider 的虚线和圆点使用 `<section>` 组合，并在装饰线内部保留零尺寸 SVG：

```html
<svg viewBox="0 0 1 1" style="float:left;line-height:0;width:0;vertical-align:top;"></svg>
```

**原因：** 微信会重新解析 inline 元素、基线和 `line-height`。零尺寸 SVG 可以稳定装饰元素的布局高度，减少线条和圆点发生基线错位的情况。

所有文章模块应调用同一个 `DividerComponent.render()`，不要在模块内部重复实现分割线。
